...
export class DatabaseSessionService {
  ...
  async addAgendaItem(
    ...
    try {
      ...
      await query(
        `INSERT INTO agenda_items (id, session_id, title, description, options, proposed_by, proposed_by_role, votes, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, to_timestamp($9 / 1000.0))`,
        [id, sessionId, title, description, JSON.stringify(options), proposedBy, 'ceo', '{}', now]
      );
  ...
  // 阶段转换
  async transitionPhase(sessionId: string, phase: MeetingPhase): Promise<boolean> {
    try {
      const session = await this.getSession(sessionId);
      if (!session) return false;
      
      await query(
        `UPDATE sessions SET phase = $1, updated_at = NOW() WHERE id = $2`,
        [phase, sessionId]
      );
  ...
  async submitVote(
    sessionId: string,
    request: { agentId: string; agendaId: string; option: number; reasoning?: string }
    ...
    try {
      ...
      // 更新议程投票
      await query(
        `UPDATE agenda_items SET votes = votes || jsonb_build_object($1::text, $2::int) WHERE id = $3`,
        [request.agentId, request.option, request.agendaId]
      );
